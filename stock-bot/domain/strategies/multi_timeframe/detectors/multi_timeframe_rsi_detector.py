# MultiTimeframeRSIDetector
# - 다중 시간대 전략에서 RSI 신호 감지를 담당하는 래퍼 클래스
# - 추후 커스텀 로직 확장/오버라이드 용이하도록 별도 래퍼로 분리
# - 기본 RSISignalDetector의 기능을 그대로 상속하며, 가중치만 전달
...
class MultiTimeframeRSIDetector(RSISignalDetector):
    """
    다중 시간대 전략(MultiTimeframeStrategy)에서 RSI 신호 감지를 담당하는 래퍼 Detector.
