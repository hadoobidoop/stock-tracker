"""
aggressive_mix 전략 조합 실행체 (독립 패키지)

- MOMENTUM(0.4), SCALPING(0.3), VOLATILITY_BREAKOUT(0.3) 전략을 가중치 기반으로 조합
- 각 하위 전략의 analyze 결과(점수, 근거 등)를 가중 평균하여 최종 신호 산출
- 임계값 조정(threshold_adjustment=0.8, 기본 8.0 → 6.4)
...
주요 파라미터:
    - config: AggressiveMixConfig (조합 전략, 가중치, 임계값 등)
...
class AggressiveMixStrategy(BaseStrategy):
    """
    aggressive_mix 전략 조합 실행체
    - MOMENTUM, SCALPING, VOLATILITY_BREAKOUT 전략을 가중치 기반으로 조합
    - 각 전략의 analyze 결과를 받아 가중 평균, 임계값 조정, 근거 통합 등 수행
    """
    def __init__(self, config: Optional[AggressiveMixConfig] = None):
        """
        Args:
            config (AggressiveMixConfig, optional): 조합 전략 config. 기본값은 AGGRESSIVE_MIX_CONFIG
        """
        super().__init__(StrategyType.AGGRESSIVE, config)
        self.config = config or AGGRESSIVE_MIX_CONFIG
    ...
    def analyze(self,
        ...
        if not self.is_initialized:
            raise RuntimeError("AggressiveMixStrategy: 하위 전략이 등록되지 않았습니다.")
    ...
    def _weighted_combination(self, individual_results: Dict[StrategyType, tuple]) -> StrategyResult:
        ...
        for strategy_type, (result, weight) in individual_results.items():
            logger.debug(f"[aggressive_mix] 하위 전략: {strategy_type}, 점수: (매수 {result.buy_score}, 매도 {result.sell_score}), 가중치: {weight}")
        ...
        logger.debug(f"[aggressive_mix] 누적 가중합: 매수 {total_weighted_buy_score}, 매도 {total_weighted_sell_score}, 총 가중치: {total_weight}")
        ...
        logger.debug(f"[aggressive_mix] 최종 가중평균: 매수 {final_buy_score}, 매도 {final_sell_score}, 신뢰도 {final_confidence}")

        # 임계값 조정
        adjusted_threshold = self.config.threshold_adjustment * 8.0  # 기본 임계값 8.0
        logger.debug(f"[aggressive_mix] 임계값(조정): {adjusted_threshold}")
        ...
        if final_buy_score > final_sell_score and final_buy_score >= adjusted_threshold:
            ...
            has_signal = True
            logger.info(f"[aggressive_mix] 매수 신호 발생! (점수: {final_buy_score} >= 임계값: {adjusted_threshold})")
        elif final_sell_score > final_buy_score and final_sell_score >= adjusted_threshold:
            final_score = final_sell_score
            has_signal = True
            logger.info(f"[aggressive_mix] 매도 신호 발생! (점수: {final_sell_score} >= 임계값: {adjusted_threshold})")
        else:
            logger.info(f"[aggressive_mix] 신호 없음 (매수: {final_buy_score}, 매도: {final_sell_score}, 임계값: {adjusted_threshold})")
