...
class BalancedSMADetector(SMASignalDetector):
    """균형 전략용 SMA 신호 감지기 - 안정적인 추세 감지"""
    
    def __init__(self, weight: float):
        super().__init__(weight)
        self.name = "Balanced_SMA_Detector"
        # Balanced 전략은 안정적인 설정 사용
        self.adx_threshold = 20  # 기본값 유지
        self.continuation_weight = 0.4  # 기본값 유지
    ...
    def detect_signals(self,
        ...
        if is_golden_cross:
            ...
            detail_msg = "Balanced SMA 골든 크로스"
            # ADX 강도에 따른 가중치 조정 (안정적인 범위)
            if adx_strength >= 25:
        ...
        # 상승 추세 지속 (표준 ADX 임계값 사용)
        elif latest_data['SMA_5'] > latest_data['SMA_20']:
            # ADX가 20 이상일 때만 추세 지속으로 인정
            if adx_strength >= self.adx_threshold:
                continuation_score = self.weight * trend_follow_buy_adj * self.continuation_weight
                detail_msg = "Balanced SMA 상승 추세 지속"
                if adx_strength >= 25:
                    continuation_score *= 1.2
                    detail_msg += f" (ADX 강세: {adx_strength:.2f})"
                elif adx_strength < self.adx_threshold:
        ...
        if is_dead_cross:
            ...
            detail_msg = "Balanced SMA 데드 크로스"
            # ADX 강도에 따른 가중치 조정 (안정적인 범위)
            if adx_strength >= 25:
        ...
        # 하락 추세 지속 (표준 ADX 임계값 사용)
        elif latest_data['SMA_5'] < latest_data['SMA_20']:
            # ADX가 20 이상일 때만 추세 지속으로 인정
            if adx_strength >= self.adx_threshold:
                continuation_score = self.weight * trend_follow_sell_adj * self.continuation_weight
                detail_msg = "Balanced SMA 하락 추세 지속"
                if adx_strength >= 25:
                    continuation_score *= 1.2
                    detail_msg += f" (ADX 강세: {adx_strength:.2f})"
                elif adx_strength < self.adx_threshold:
