...
try:
    from domain.strategies.single.multi_timeframe.configs.multi_timeframe_config import MULTI_TIMEFRAME_CONFIG
except ImportError:
...
try:
    from domain.strategies.dynamic.market_regime_hybrid.configs.market_regime_hybrid_config import MARKET_REGIME_HYBRID_CONFIG
except ImportError:
    MARKET_REGIME_HYBRID_CONFIG = None
from domain.strategies.base import BaseStrategy
from domain.strategies.dynamic.conservative_reversion_hybrid.conservative_reversion_hybrid_strategy import ConservativeReversionHybridStrategy
from domain.strategies.dynamic.adaptive_momentum_hybrid.adaptive_momentum_hybrid_strategy import AdaptiveMomentumStrategy
from domain.strategies.dynamic.market_regime_hybrid.market_regime_hybrid_strategy import MarketRegimeHybridStrategy
...
from domain.strategies.single.scalping.scalping_strategy import ScalpingStrategy
from domain.strategies.single.swing.swing_strategy import SwingStrategy
from domain.strategies.single.trend_following.trend_following_strategy import TrendFollowingStrategy
from domain.strategies.single.trend_pullback.trend_pullback_strategy import TrendPullbackStrategy
from domain.strategies.single.mean_reversion.mean_reversion_strategy import MeanReversionStrategy
from domain.strategies.single.multi_timeframe.multi_timeframe_strategy import MultiTimeframeStrategy
from domain.strategies.dynamic.dynamic_strategy_manager.dynamic_strategy import DynamicCompositeStrategy
from domain.strategies.dynamic.dynamic_strategy_manager.modifiers.registry import ModifierFactory
...
class StrategyFactory:
    ...
    def create_dynamic_strategy(cls, strategy_name: str) -> Optional[BaseStrategy]:
        ...
        try:
            ...
            # 1. ModifierEngine 생성
            modifier_definitions = get_all_modifiers()  # Changed back to get_all_modifiers
