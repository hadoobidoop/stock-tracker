...
def get_ohlcv_data(symbols: Union[str, List[str]], period: str, interval: str) -> Union[
    ...
    try:
        ...
        if not isinstance(df_raw, pd.DataFrame) or df_raw.empty:
            logger.warning(
                f"No data fetched or invalid data format for {symbols_list} with period {period} and interval {interval}. Returned type: {type(df_raw)}.")
        ...
        if isinstance(df_raw.columns, pd.MultiIndex):
                ...
                try:
                    ...
                    for col in required_cols:
                        if col not in df_symbol.columns:
                            logger.error(
                                f"Missing required column '{col}' for {ticker} from Yahoo Finance data. Available columns: {df_symbol.columns.tolist()}")
                    ...
                    logger.warning(
                        f"Ticker {ticker} not found in fetched multi-index columns: {ke}. Skipping {ticker}.")
            ...
            for col in required_cols:
                if col not in df_raw.columns:
                    logger.error(
                        f"Missing required column '{col}' for {symbols_list[0]} from Yahoo Finance data. Available columns: {df_raw.columns.tolist()}")
                    return pd.DataFrame()
