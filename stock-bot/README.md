...
백테스팅의 정확도를 높이기 위해, `scripts/backfill_market_data.py`를 사용하여 과거의 거시 경제 지표 데이터를 미리 채워 넣을 수 있습니다.

**1. 모든 지표를 특정 기간 동안 백필:**
```bash
python scripts/backfill_market_data.py --start-date 2020-01-01 --end-date 2023-12-31
```

**2. 특정 지표만 골라서 백필:**
```bash
python scripts/backfill_market_data.py --start-date 2020-01-01 --end-date 2023-12-31 --indicators VIX FEAR_GREED_INDEX
```
