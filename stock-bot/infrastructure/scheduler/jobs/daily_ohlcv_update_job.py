...
def daily_ohlcv_update_job():
    """
    야후 파이낸스에서 일봉 데이터를 가져와 DB에 저장하는 스케줄링 작업입니다.
    DB에 저장된 모든 분석 대상 종목에 대해 페이징 처리하여 실행합니다.
    """
    logger.info("JOB START: Daily OHLCV data update from Yahoo Finance...")
    ...
    # 설정에서 파라미터 가져오기
    daily_settings = OHLCV_COLLECTION["DAILY"]
    api_control_settings = OHLCV_COLLECTION["API_CONTROL"]
    period = daily_settings["PERIOD"]
    interval = daily_settings["INTERVAL"]
    days_to_keep = daily_settings["DAYS_TO_KEEP"]
    ...
    logger.info(f"Starting daily OHLCV update for {total_stocks} stocks in {total_pages} pages...")
    ...
    for page in range(1, total_pages + 1):
        ...
        try:
            ...
            if not successful_data:
                logger.error("No data retrieved from Yahoo Finance for this page.")
                continue
            ...
            for ticker, df in successful_data.items():
                ...
                df_clean = df.dropna()
                if df_clean.empty:
                    logger.warning(f"No valid data for {ticker} after cleaning")
                    continue
                ...
                if not df_filtered.empty:
                    ...
                    logger.info(f"Prepared {len(df_filtered)} daily records for {ticker}")
            
            if not validated_data:
                logger.error("No valid data to save after validation for this page.")
                continue
            ...
            if save_success:
                ...
                logger.info(f"Page {page}/{total_pages}: Successfully saved/updated {total_records} daily OHLCV records.")
            else:
                logger.error(f"Page {page}/{total_pages}: Failed to save daily OHLCV data.")
    ...
    logger.info("JOB END: Daily OHLCV data update process finished.")
