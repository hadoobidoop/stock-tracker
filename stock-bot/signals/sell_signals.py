...
def detect_sell_signals(df_intraday: pd.DataFrame, ticker: str, market_trend: TrendType,
                       long_term_trend: TrendType, daily_extra_indicators: dict = None) -> tuple[float, list]:
    """
    매도 신호를 감지하고 점수를 계산합니다.
    ...
    Returns:
        tuple[float, list]: (매도 점수, 매도 신호 상세 정보 리스트)
    """
    if df_intraday.empty or len(df_intraday) < 2:
        logger.warning(f"Not enough intraday data for sell signal detection for {ticker}.")
        return 0.0, []
    ...
    if market_trend == "BULLISH":
        ...
        logger.debug(
            f"Adjusting SELL signal threshold for {ticker} in BULLISH market to {adjusted_signal_threshold:.2f}.")
    elif market_trend == "BEARISH":
        adjusted_signal_threshold = SIGNAL_THRESHOLD * 0.8
        logger.debug(
            f"Adjusting SELL signal threshold for {ticker} in BEARISH market to {adjusted_signal_threshold:.2f}.")
    ...
    if prev_data['SMA_5'] > prev_data['SMA_20'] and latest_data['SMA_5'] < latest_data['SMA_20']:
        sma_cross_sell_score = SIGNAL_WEIGHTS["dead_cross_sma"] * trend_follow_sell_adj
    ...
    if (prev_data['MACD_12_26_9'] > prev_data['MACDs_12_26_9'] and latest_data['MACD_12_26_9'] < latest_data['MACDs_12_26_9']) and \
        ...
        sell_details.append(f"MACD 데드 크로스 & 거래량 급증 확인 (MACD:{latest_data['MACD_12_26_9']:.2f}, Volume:{latest_data['Volume']:,})")
    ...
    if latest_data['RSI_14'] >= 70:
        if latest_data['Close'] >= latest_data['BBU_20_2.0']:
            if prev_data['Close'] < prev_data['BBU_20_2.0'] or \
                ...
                sell_details.append(f"RSI 과매수 & BB 상단 반전 (RSI:{latest_data['RSI_14']:.2f}, Close:{latest_data['Close']:.2f} vs BBU:{latest_data['BBU_20_2.0']:.2f})")
    ...
    if latest_data['Volume'] > latest_data['Volume_SMA_20'] * VOLUME_SURGE_FACTOR:
        ...
        sell_details.append(f"거래량 급증 (현재:{latest_data['Volume']} > 평균:{latest_data['Volume_SMA_20']:.0f} * {VOLUME_SURGE_FACTOR})")
    ...
    return sell_score, sell_details 