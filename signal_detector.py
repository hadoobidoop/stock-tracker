...
def detect_weighted_signals(df_intraday: pd.DataFrame, ticker: str, market_trend: str = "NEUTRAL", daily_extra_indicators: dict = None) -> dict:
    ...
    if market_trend == "BEARISH":
        ...
        logger.debug(f"Adjusting BUY signal threshold for {ticker} in BEARISH market to {adjusted_signal_threshold:.2f}.")
    ...
    if (prev_data['MACD_12_26_9'] > prev_data['MACDs_12_26_9'] and latest_data['MACD_12_26_9'] < latest_data['MACDs_12_26_9']) and \
            (latest_data['Volume'] > latest_data['Volume_SMA_20'] * VOLUME_SURGE_FACTOR and latest_data['Close'] < prev_data['Close']):
        sell_score += SIGNAL_WEIGHTS["macd_volume_confirm"] * volume_adj
        sell_details.append(f"MACD 데드 크로스 & 하락 거래량 급증 확인 (MACD:{latest_data['MACD_12_26_9']:.2f}, Volume:{latest_data['Volume']:,})")
    # --- 새로운 혼합 지표 패턴 끝 ---
    ...
    if latest_data['RSI_14'] >= 70:
        if latest_data['Close'] >= latest_data['BBU_20_2.0']:
            if prev_data['Close'] < prev_data['BBU_20_2.0'] or \
                    (prev_data['Close'] >= prev_data['BBU_20_2.0'] and latest_data['Close'] < latest_data['BBU_20_2.0']):
                sell_score += SIGNAL_WEIGHTS["rsi_bb_reversal"] * momentum_reversal_adj
                sell_details.append(f"RSI 과매수 & BB 상단 반등 (RSI:{latest_data['RSI_14']:.2f}, Close:{latest_data['Close']:.2f} vs BBU:{latest_data['BBU_20_2.0']:.2f})")
    # --- 혼합 지표 패턴 끝 ---
    ...
    if (prev_data['RSI_14'] >= 70 > latest_data['RSI_14']) and \
            (prev_data['STOCHk_14_3_3'] > prev_data['STOCHd_14_3_3'] and latest_data['STOCHk_14_3_3'] < latest_data['STOCHd_14_3_3'] and latest_data['STOCHk_14_3_3'] > 20):
        sell_score += SIGNAL_WEIGHTS["rsi_stoch_confirm"] * momentum_reversal_adj
        sell_details.append(f"RSI/Stoch 동시 매도 신호 (RSI:{latest_data['RSI_14']:.2f}, Stoch %K:{latest_data['STOCHk_14_3_3']:.2f})")
    # --- 새로운 혼합 지표 패턴 끝 ---
    ...
    if daily_extra_indicators:
        ...
        if 'R1' in pivot_points and abs(latest_data['Close'] - pivot_points['R1']) <= proximity_threshold:
            if (prev_data['RSI_14'] >= 70 > latest_data['RSI_14']) or \
                ...
                sell_details.append(f"Pivot R1 & 모멘텀 반전 (R1:{pivot_points['R1']:.2f}, Close:{latest_data['Close']:.2f})")

        if 'R2' in pivot_points and abs(latest_data['Close'] - pivot_points['R2']) <= proximity_threshold:
            if (prev_data['RSI_14'] >= 70 > latest_data['RSI_14']) or \
                    (prev_data['STOCHk_14_3_3'] > prev_data['STOCHd_14_3_3'] and latest_data['STOCHk_14_3_3'] < latest_data['STOCHd_14_3_3'] and latest_data['STOCHk_14_3_3'] > 20):
                sell_score += SIGNAL_WEIGHTS["pivot_momentum_reversal"] * pivot_fib_adj
                sell_details.append(f"Pivot R2 & 모멘텀 반전 (R2:{pivot_points['R2']:.2f}, Close:{latest_data['Close']:.2f})")
        ...
        for fib_level_key in ['38.2%', '50.0%', '61.8%']:
            if fib_level_key in fib_retracement_levels and abs(latest_data['Close'] - fib_retracement_levels[fib_level_key]) <= proximity_threshold:
                if (prev_data['RSI_14'] >= 70 > latest_data['RSI_14']) or \
                    ...
                    sell_details.append(f"Fib {fib_level_key} & 모멘텀 반전 (Fib:{fib_retracement_levels[fib_level_key]:.2f}, Close:{latest_data['Close']:.2f})")
    # --- 새로운 혼합 지표 패턴 끝 ---
    ...
    # --- 최종 신호 판단 ---
    # Buy 신호 점수가 adjusted_signal_threshold 이상이고, Sell 신호 점수보다 높을 때
    final_stop_loss_price = None
    if buy_score >= adjusted_signal_threshold and buy_score > sell_score:
        # 매수 신호 시 손절매 가격 계산 (현재 종가 - 2 * ATR)
        if current_atr > 0:
            final_stop_loss_price = latest_data['Close'] - (current_atr * 2) # 2배 ATR
            if final_stop_loss_price < 0: # 손절매 가격이 0보다 작아지는 것을 방지
                final_stop_loss_price = 0.01 # 최소값 설정

        logger.info(f"Strong BUY signal detected for {ticker} (Score: {buy_score}). Market Trend: {market_trend}. Adjusted Threshold: {adjusted_signal_threshold:.2f}. Stop Loss: {final_stop_loss_price:.2f}.")
        return {
            'type': 'BUY',
            'score': buy_score,
            'details': buy_details,
            ...
            'stop_loss_price': final_stop_loss_price # 손절매 가격 포함
        }
    # Sell 신호 점수가 adjusted_signal_threshold 이상이고, Buy 신호 점수보다 높을 때
    elif sell_score >= adjusted_signal_threshold and sell_score > buy_score:
        # 매도 신호 시 손절매 가격 계산 (현재 종가 + 2 * ATR)
        if current_atr > 0:
            final_stop_loss_price = latest_data['Close'] + (current_atr * 2) # 2배 ATR

        logger.info(f"Strong SELL signal detected for {ticker} (Score: {sell_score}). Market Trend: {market_trend}. Adjusted Threshold: {adjusted_signal_threshold:.2f}. Stop Loss: {final_stop_loss_price:.2f}.")
        return {
            'type': 'SELL',
            'score': sell_score,
        ...
        logger.debug(f"No strong signal for {ticker}. Buy Score: {buy_score}, Sell Score: {sell_score}. Market Trend: {market_trend}. Adjusted Threshold: {adjusted_signal_threshold:.2f}.")
        return {}